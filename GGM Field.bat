...
:: It's a lightweight field companion. Node 1 is king.
:: ═══════════════════════════════════════════════════════════════
    ...
    if "!HAS_INTERNET!"=="1" (
        ...
        if "!BEHIND!"=="0" (
            ...
            if !errorlevel! equ 0 (
                ...
                echo.
                echo         Changed files:
                git diff --stat !OLD_HASH!..HEAD 2>nul | findstr /v "changed" | findstr /v "^$"
            ) else (
                echo         WARNING: Pull failed — you may have local changes.
                echo         Try manually: git stash ^&^& git pull ^&^& git stash pop
            )
        )
    ) else (
...
if "!SHEETS_OK!"=="1" (
    for /f "tokens=*" %%s in ('"!PYTHON!" -c "import requests,json; r=requests.get(\"https://script.google.com/macros/s/AKfycbx-q2qSeCorIEeXPE9d2MgAZLKEFwFNW9lARLE1yYciH9wJWwvktUTuDVLz_rSCbUhkMg/exec?action=get_node_status\",timeout=15,allow_redirects=True); data=r.json(); nodes=data if isinstance(data,list) else data.get(\"data\",data.get(\"nodes\",[])); pc=[n for n in nodes if \"pc\" in str(n.get(\"node_id\",\"\")).lower() or \"hub\" in str(n.get(\"node_id\",\"\")).lower()]; print(\"ONLINE\" if pc else \"OFFLINE\")" 2^>nul') do set "PC_STATUS=%%s"
    if "!PC_STATUS!"=="ONLINE" (
...
:: Read version from field_app.py
set "APP_VER=?.?.?"
...
:: ══════════════════════════════════════════════════════════════
:: SHUTDOWN — App window was closed
:: ══════════════════════════════════════════════════════════════
echo.
if !errorlevel! neq 0 (
