...
function doGet(e) {
  ...
  try {
    switch (action) {
      ...
      // ── Site Banners ──
      case 'get_site_banners':
        return jsonResponse(handleGetSiteBanners(params));

      // ── System ──
      case 'health_check':
        return jsonResponse({ status: 'success', version: CONFIG.VERSION, timestamp: new Date().toISOString() });
      case 'run_health_check':
        return jsonResponse(runHealthCheck());
      case 'sheet_read':
...
function doPost(e) {
  ...
  try {
    switch (action) {
      ...
      // ── Site Banners ──
      case 'set_site_banner':
        return jsonResponse(handleSetSiteBanner(data));
      case 'delete_site_banner':
        return jsonResponse(handleDeleteSiteBanner(data));
      case 'run_health_check':
        return jsonResponse(runHealthCheck());
...
// ═══════════════════════════════════════════════════════════════
// SITE BANNERS — Scrolling announcement system
// ═══════════════════════════════════════════════════════════════

var BANNER_HEADERS = [
  'ID', 'Message', 'Type', 'Active', 'Priority',
  'Link', 'LinkText', 'CreatedBy', 'CreatedAt', 'ExpiresAt', 'AutoGenerated'
];

/**
 * GET: Return all active, non-expired banners sorted by priority.
 * Called by the website every 60s to check for announcements.
 * Params: ?includeInactive=true to get all (for admin UI)
 */
function handleGetSiteBanners(params) {
  var sheet = getOrCreateSheet('SiteBanners', BANNER_HEADERS);
  var rows = sheet.getDataRange().getValues();
  if (rows.length < 2) return { status: 'success', banners: [] };

  var headers = rows[0];
  var includeAll = (params && params.includeInactive === 'true');
  var now = new Date();
  var banners = [];
  ...
  for (var i = 1; i < rows.length; i++) {
    ...
    // Skip inactive unless admin requested all
    if (!includeAll && String(obj.Active).toLowerCase() !== 'true') continue;
    // Skip expired
    if (obj.ExpiresAt && new Date(obj.ExpiresAt) < now) {
      // Auto-deactivate expired banners
      sheet.getRange(i + 1, headers.indexOf('Active') + 1).setValue('FALSE');
      continue;
    }
    banners.push(obj);
  }
  ...
  banners.sort(function(a, b) { return (Number(b.Priority) || 0) - (Number(a.Priority) || 0); });

  return { status: 'success', banners: banners };
}

/**
 * POST: Create or update a site banner.
 * data: { id?, message, type, active?, priority?, link?, linkText?, createdBy, expiresAt?, autoGenerated? }
 * Types: info | warning | error | maintenance | payment | booking
 */
function handleSetSiteBanner(data) {
  var sheet = getOrCreateSheet('SiteBanners', BANNER_HEADERS);
  var id = data.id || Utilities.getUuid();
  var now = new Date().toISOString();
  var message = data.message || '';
  var type = data.type || 'info';
  var active = data.active !== undefined ? String(data.active) : 'TRUE';
  var priority = data.priority || 5;
  var link = data.link || '';
  var linkText = data.linkText || '';
  var createdBy = data.createdBy || 'system';
  var expiresAt = data.expiresAt || '';
  var autoGenerated = data.autoGenerated ? 'TRUE' : 'FALSE';

  if (!message) return { status: 'error', message: 'Banner message is required' };
  ...
  if (data.id) {
    ...
    for (var i = 1; i < rows.length; i++) {
      if (rows[i][0] === data.id) {
        var rowData = [id, message, type, active, priority, link, linkText, createdBy, rows[i][8], expiresAt, autoGenerated];
        sheet.getRange(i + 1, 1, 1, rowData.length).setValues([rowData]);
        logActivity('banner_updated', { id: id, message: message, type: type });
  ...
  // New banner
  sheet.appendRow([id, message, type, active, priority, link, linkText, createdBy, now, expiresAt, autoGenerated]);
  logActivity('banner_created', { id: id, message: message, type: type, createdBy: createdBy });

  // Telegram notification
  var emoji = type === 'error' ? '🔴' : type === 'warning' ? '🟡' : type === 'maintenance' ? '🔧' : 'ℹ️';
  notifyTelegram(emoji + ' *Site Banner Published*\n' + message + '\nType: ' + type + ' | By: ' + createdBy);

  return { status: 'success', id: id, action: 'created' };
}

/**
 * POST: Delete (deactivate) a site banner.
 * data: { id }
 */
function handleDeleteSiteBanner(data) {
  if (!data.id) return { status: 'error', message: 'Banner ID required' };
  var sheet = getOrCreateSheet('SiteBanners', BANNER_HEADERS);
  var rows = sheet.getDataRange().getValues();
  for (var i = 1; i < rows.length; i++) {
    if (rows[i][0] === data.id) {
      sheet.deleteRow(i + 1);
      logActivity('banner_deleted', { id: data.id });
      return { status: 'success', id: data.id, action: 'deleted' };
    }
  }
...
/**
 * Auto-banner: Called by health checks to set/clear system status banners.
 * If the issue is resolved, it deactivates the auto-generated banner.
 * type: 'payment' | 'booking' | 'email' | 'maintenance'
 */
function autoSetHealthBanner(type, message, active) {
  var sheet = getOrCreateSheet('SiteBanners', BANNER_HEADERS);
  var rows = sheet.getDataRange().getValues();
  var headers = rows[0];
  var typeCol = headers.indexOf('Type');
  var autoCol = headers.indexOf('AutoGenerated');
  var activeCol = headers.indexOf('Active');
  var msgCol = headers.indexOf('Message');

  // Find existing auto-banner for this type
  for (var i = 1; i < rows.length; i++) {
    if (rows[i][typeCol] === type && String(rows[i][autoCol]).toLowerCase() === 'true') {
      // Update existing
      sheet.getRange(i + 1, activeCol + 1).setValue(active ? 'TRUE' : 'FALSE');
      if (message) sheet.getRange(i + 1, msgCol + 1).setValue(message);
      return { updated: true, row: i + 1 };
    }
  }

  // Create new auto-banner if activating
  if (active) {
    var id = Utilities.getUuid();
    var now = new Date().toISOString();
    // Auto-expire in 24 hours
    var expires = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString();
    sheet.appendRow([id, message, type, 'TRUE', 8, '', '', 'health-check', now, expires, 'TRUE']);
    notifyTelegram('🔴 *Auto-Banner Activated*\n' + message + '\nType: ' + type);
...
/**
 * Automated health check — tests key systems and auto-manages banners.
 * Can be called via GAS time trigger (every 15 min) or from PC Hub.
 * GET: ?action=run_health_check
 * POST: { action: "run_health_check" }
 *
 * Tests:
 *   1. Stripe API connectivity
 *   2. Email (Brevo) connectivity
 *   3. Spreadsheet read/write access
 *   4. Recent booking flow (any bookings in last 7 days?)
 *
 * Customer-facing messages are kept simple and non-technical.
 */
function runHealthCheck() {
  var results = { timestamp: new Date().toISOString(), checks: {} };

  // 1. Stripe connectivity
  try {
    if (CONFIG.STRIPE_SECRET_KEY) {
      var stripeResp = UrlFetchApp.fetch('https://api.stripe.com/v1/balance', {
        method: 'get',
        headers: { 'Authorization': 'Bearer ' + CONFIG.STRIPE_SECRET_KEY },
        muteHttpExceptions: true
      });
      var stripeOk = stripeResp.getResponseCode() === 200;
      results.checks.stripe = { ok: stripeOk, code: stripeResp.getResponseCode() };
      if (!stripeOk) {
        autoSetHealthBanner('payment',
          'We are currently experiencing issues with our payment system. You can still book — just select "Pay Later" and we will arrange payment separately.',
          true);
      } else {
        autoSetHealthBanner('payment', '', false);
      }
    } else {
      results.checks.stripe = { ok: false, reason: 'no_key' };
    }
  } catch (e) {
    results.checks.stripe = { ok: false, error: e.message };
    autoSetHealthBanner('payment',
      'Our online payment system is temporarily unavailable. You can still book your service — just select "Pay Later" and we will be in touch to arrange payment.',
      true);
  }

  // 2. Brevo (email) connectivity
  try {
    var brevoKey = PropertiesService.getScriptProperties().getProperty('BREVO_API_KEY');
    if (brevoKey) {
      var brevoResp = UrlFetchApp.fetch('https://api.brevo.com/v3/account', {
        method: 'get',
        headers: { 'api-key': brevoKey, 'accept': 'application/json' },
        muteHttpExceptions: true
      });
      var emailOk = brevoResp.getResponseCode() === 200;
      results.checks.email = { ok: emailOk, code: brevoResp.getResponseCode() };
      if (!emailOk) {
        autoSetHealthBanner('maintenance',
          'Our email notifications are temporarily delayed. Your booking is still confirmed — we will be in touch shortly.',
          true);
      } else {
        autoSetHealthBanner('maintenance', '', false);
      }
    } else {
      results.checks.email = { ok: true, reason: 'no_key_configured' };
    }
  } catch (e) {
    results.checks.email = { ok: false, error: e.message };
    autoSetHealthBanner('maintenance',
      'Our email notifications are temporarily delayed. Your booking is still confirmed — we will be in touch shortly.',
      true);
  }
  ...
  try {
    var ss = getSpreadsheet();
    var testSheet = ss.getSheetByName('Bookings');
    results.checks.spreadsheet = { ok: !!testSheet, sheets: ss.getSheets().length };
  } catch (e) {
    results.checks.spreadsheet = { ok: false, error: e.message };
    autoSetHealthBanner('error',
      'We are experiencing a temporary technical issue. Our team has been notified and is working on it. Please try again shortly.',
      true);
  }

  // 4. Recent booking activity (sanity check)
  try {
    var bookingSheet = getSpreadsheet().getSheetByName('Bookings');
    if (bookingSheet && bookingSheet.getLastRow() > 1) {
      var lastRow = bookingSheet.getRange(bookingSheet.getLastRow(), 1, 1, 5).getValues()[0];
      results.checks.recentBookings = { ok: true, lastEntry: lastRow[0] };
    } else {
      results.checks.recentBookings = { ok: true, note: 'no_bookings_sheet' };
    }
  } catch (e) {
    results.checks.recentBookings = { ok: false, error: e.message };
  }

  // Overall status
  var allOk = Object.keys(results.checks).every(function(k) { return results.checks[k].ok; });
  results.status = allOk ? 'healthy' : 'degraded';

  // If everything is healthy, clear any lingering auto-banners
  if (allOk) {
    autoSetHealthBanner('payment', '', false);
    autoSetHealthBanner('maintenance', '', false);
    autoSetHealthBanner('error', '', false);
    autoSetHealthBanner('booking', '', false);
  }

  logActivity('health_check', results);
