...
class FieldApp(ctk.CTk):
    ...
    def _render_dashboard(self, jobs, events, tracking, finance, enquiries,
        ...
        if not jobs:
            ...
            for j in jobs[:8]:
                ...
                if maps_url:
                    ctk.CTkButton(acts, text="🏠", height=22, width=30,
        ...
        if unpaid_count > 0:
            alerts.append((f"💸 {unpaid_count} unpaid invoice(s) (£{outstanding:,.0f}) ÔåÆ", C["danger"], "finance"))
        if pending_enq > 0:
            alerts.append((f"📩 {pending_enq} new enquir{'ies' if pending_enq > 1 else 'y'} ÔåÆ", C["warning"], "enquiries"))
        if pending_quotes > 0:
    ...
    def _render_job_card(self, parent, job, idx):
        ...
        if maps_url:
            ctk.CTkButton(actions, text="🏠", height=26, width=36,
    ...
    def _en_route_job(self, ref):
        try:
            ...
            self._set_status(f"🚗 En route ÔåÆ {ref}")
    ...
    def _render_health(self, checks):
        ...
        if fails > 0:
            ...
            banner_text = f"⚠️  {warns} WARNING(S)  —  {passes} passed"
